  ...
  if (error) {
    return (
      <div className={`relative w-full md:w-auto ${className}`}>
        <div className="text-sm text-red-500">Error fetching achievements. Please try again later.</div>
      </div>
    );
  }
  ...
  return (
    <div className={`relative w-full md:w-auto ${className}`}>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgb(251, 191, 36) 0%, rgb(245, 158, 11) 50%, rgb(217, 119, 6) 100%)",
          boxShadow:
            "rgba(251, 191, 36, 0.4) 0px 10px 25px -5px, rgba(255, 255, 255, 0.1) 0px 0px 0px 1px inset",
        }}
      ...
      >
        ...
        >
          ...
          <span className="relative z-10">{isLoading ? "Loading..." : latestAchievement ? latestAchievement.name : "No Achievement"}</span>
        </a>
        ...
        <button
          ...
          style={{
            background:
              "linear-gradient(135deg, rgb(245, 158, 11) 0%, rgb(217, 119, 6) 50%, rgb(180, 83, 9) 100%)",
            textShadow: "rgba(0, 0, 0, 0.3) 0px 1px 2px",
          }}
          onClick={() => setIsOpen(!isOpen)}
        >
      ...
      {isOpen && (
        <div
          className="hackathon-dropdown bg-white dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg shadow-2xl p-3 backdrop-blur-sm"
          style={{
            position: "absolute",
            top: "calc(100% + 8px)", // Positions the dropdown directly below the badge
            left: "0",
            zIndex: 2147483647,
            width: "100%", // Make the dropdown width match the button
...
>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-1 min-w-0">
                      ...
                      <span className="text-[8px] bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded-full font-medium whitespace-nowrap flex-shrink-0">
                        {achievement.type}
                      </span>
                    </div>
                  </div>
