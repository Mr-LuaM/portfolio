    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
        {displayedProjects?.map((project) => {
          const { id, name, description, project_url, repo_url, technologies } = project;
...
className={`flex items-center justify-center rounded-md transition-colors
        bg-gray-100 hover:bg-gray-200 dark:bg-black dark:text-white hover:dark:bg-neutral-800
        ${isProjectPage ? 'p-2' : 'p-1.5'}`}    >
      <svg  className={isProjectPage ? 'w-5 h-5' : 'w-4 h-4'} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
  ...
  {repo_url && project.access_type === "public" && (
    <a
      href={repo_url}
      target="_blank"
      rel="noopener noreferrer"
...
className={`flex items-center justify-center rounded-md transition-colors
        bg-gray-100 hover:bg-gray-200 dark:bg-black dark:text-white hover:dark:bg-neutral-800
        ${isProjectPage ? 'p-2' : 'p-1.5'}`}    >
      <svg         className={isProjectPage ? 'w-5 h-5' : 'w-4 h-4'}
